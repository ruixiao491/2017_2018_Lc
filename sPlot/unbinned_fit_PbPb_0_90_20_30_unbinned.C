...
void unbinned_fit_PbPb_0_90_20_30_unbinned(TString nt_number, TString outfile, TString outfigure,int range1, int range2){
/////PbPb cen0_100%

TFile *f =  new TFile("/scratch/brown/xiao147/2018PbPb_rereco_data_Dntuple_05052020_firstturn/sPlot_brown/10_30_sPlot_masstuples_brown_halstead_whole.root");
...
double mean_value = 2.28561e+00;
double float_width_parameter = 2.41150e-01;
TH1F *h_output = new TH1F ("h_output","output",2,0,2);



TNtuple *nt = (TNtuple*)f->Get(nt_number);
TH1F *hb = new TH1F("hb","hb",42,2.108,2.444);
nt->Draw("mass>>hb");
//TH1F *hb = (TH1F*)f->Get("alpha_bin_10");
//hb->Draw("e");
double numbers = 4.95257e+03;
...
massframe->SetAxisRange(range1,range2,"Y");
massframe->Draw();
...
tex = new TLatex(0.16,0.66,Form("Yield: %i^{ + %i}_{ - %i} ",a,c,b));
tex->SetNDC();
tex->SetTextFont(42);
tex->SetTextSize(0.047);
tex->SetLineWidth(2);
...
RooRealVar* sigYield = w->var("NumSig");
RooRealVar* backYield = w->var("NumBkg");
RooDataSet* data = (RooDataSet*) w->data("ds");
cout<<"Singal number: "<<sigYield->getVal()<<endl;
Model->fitTo(*data, Extended(),Range(2.108,2.444),Minos(kTRUE));

RooRealVar* Mean = w->var("mean");
RooRealVar* Width1 = w->var("width1");
RooRealVar* Width2 = w->var("width2");
RooRealVar* Width3 = w->var("width3");
RooRealVar* Ratio1 = w->var("ratio1");
RooRealVar* Ratio2 = w->var("ratio2");
RooRealVar* P1 = w->var("p1");
RooRealVar* P2 = w->var("p2");
RooRealVar* P3 = w->var("p3");
RooRealVar* Float_width = w->var("float_width");
Mean->setConstant();
Width1->setConstant();
Width2->setConstant();
Width3->setConstant();
...
RooStats::SPlot* sData = new RooStats::SPlot("sData","An SPlot",
                    *data, Model, RooArgList(*sigYield,*backYield) );
std::cout<<" Check Sweight:  "<<std::endl;
std::cout<< std::endl<<"Yield of signal is "<<NumSig.getVal()<<". From sWeights it is "<<sData->GetYieldFromSWeight("NumSig")<<std::endl;
std::cout<<"Yield of background is  "<<NumBkg.getVal()<<". From sWeights it is "<<sData->GetYieldFromSWeight("NumBkg")<<std::endl;
...
TTree *sig_weight_branch = new TTree("sig_weight_branch","sig_weight_branch");
sig_weight_branch->Branch("sig_weight",&sig_weight,"sig_weight/D");

int NEntriesData = data->numEntries();
cout << "NEntriesData = " << NEntriesData << endl;

for (int i=0; i<NEntriesData; i++)
{
	    //std::cout<<"signal weight      "<<sData->GetSWeight(i,"NumSig")<<"      "<<"background weight         "<<sData->GetSWeight(i,"NumBkg")<<"         "<<"Total weight      "<<sData->GetSumOfEventSWeight(i)<<std::endl;

		sig_weight = sData->GetSWeight(i,"NumSig");
		sig_weight_branch->Fill();

}
sig_weight_branch->Write();
outf->Close();
