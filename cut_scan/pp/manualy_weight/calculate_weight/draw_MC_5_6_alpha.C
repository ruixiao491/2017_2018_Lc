...
int draw_MC_5_6_alpha(TString infile="/scratch/halstead/x/xiao147/official_2017_18_MC/pp_Dntuple/pp_pt4_MC_Dntuple/with_geninfo_nonprompt_info/Fill_BDTG_3_100/pkpi_pt4_MC_BDTG_pT3_100_5_cutvariable_0_1_2_3_5_DATA.root",TString outfile="test.root",Bool_t iseos=false)
{
	...
	TH1 *h_alpha_4_5 = new TH1D("h_alpha_4_5","h_alpha_4_5",NBINS,edges);
	TH1 *h_alpha_5_6 = new TH1D("h_alpha_5_6","h_alpha_5_6",NBINS,edges);
	TH1 *h_alpha_6_8 = new TH1D("h_alpha_6_8","h_alpha_6_8",NBINS,edges);
	TH1 *h_alpha_8_10 = new TH1D("h_alpha_8_10","h_alpha_8_10",NBINS,edges);
	TH1 *h_alpha_10_125 = new TH1D("h_alpha_10_125","h_alpha_10_125",NBINS,edges);
	TH1 *h_alpha_125_15 = new TH1D("h_alpha_125_15","h_alpha_125_15",NBINS,edges);
	TH1 *h_alpha_15_20 = new TH1D("h_alpha_15_20","h_alpha_15_20",NBINS,edges);
	TH1 *h_alpha_10_20 = new TH1D("h_alpha_10_20","h_alpha_10_20",NBINS,edges);
	...
	for (int entry = 0; entry < t2->GetEntries(); entry++)
	{
		...
		if (abs(PVz)<15&&pBeamScrapingFilter&&pPAprimaryVertexFilter)
		{
			...
			{
				if(Dpt[i]>4&&Dpt[i]<5&&Dtrk1PtErr[i]/Dtrk1Pt[i]<0.3&&Dtrk2PtErr[i]/Dtrk2Pt[i]<0.3&&Dtrk3PtErr[i]/Dtrk3Pt[i]<0.3&&abs(Dtrk1Eta[i])<1.2&&abs(Dtrk2Eta[i])<1.2&&abs(Dtrk3Eta[i])<1.2&&abs(Dy[i])<1&&Dtrk1highPurity[i]==1&&Dtrk2highPurity[i]==1&&Dtrk3highPurity[i]==1&&DlxyBS[i]/sqrt(DlxyBSErr[i])>dlscut&&(Dtrk1PixelHit[i]+Dtrk1StripHit[i])>=11&&(Dtrk2PixelHit[i]+Dtrk2StripHit[i])>=11&&(Dtrk3PixelHit[i]+Dtrk3StripHit[i])>=11&&Dgen[i]==23333&&Dchi2cl[i]>chi2cut)
				{
					weight=data_fit_fun->Eval(Dgenpt[i])/MC_fit_fun->Eval(Dgenpt[i]);
					h_alpha_4_5->Fill(alpha_BS_2D_correction[i],weight);
				}
				...
				{
					weight=data_fit_fun->Eval(Dgenpt[i])/MC_fit_fun->Eval(Dgenpt[i]);
					h_alpha_5_6->Fill(alpha_BS_2D_correction[i],weight);
				}
				...
				{
					weight=data_fit_fun->Eval(Dgenpt[i])/MC_fit_fun->Eval(Dgenpt[i]);
					h_alpha_6_8->Fill(alpha_BS_2D_correction[i],weight);
				}
				...
				{
					weight=data_fit_fun->Eval(Dgenpt[i])/MC_fit_fun->Eval(Dgenpt[i]);
					h_alpha_8_10->Fill(alpha_BS_2D_correction[i],weight);
				}
				...
				{
					weight=data_fit_fun->Eval(Dgenpt[i])/MC_fit_fun->Eval(Dgenpt[i]);
					h_alpha_10_125->Fill(alpha_BS_2D_correction[i],weight);
				}
				...
				{
					weight=data_fit_fun->Eval(Dgenpt[i])/MC_fit_fun->Eval(Dgenpt[i]);
					h_alpha_125_15->Fill(alpha_BS_2D_correction[i],weight);
				}
				...
				{
					weight=data_fit_fun->Eval(Dgenpt[i])/MC_fit_fun->Eval(Dgenpt[i]);
					h_alpha_15_20->Fill(alpha_BS_2D_correction[i],weight);
				}

				if(Dpt[i]>10&&Dpt[i]<20&&Dtrk1PtErr[i]/Dtrk1Pt[i]<0.3&&Dtrk2PtErr[i]/Dtrk2Pt[i]<0.3&&Dtrk3PtErr[i]/Dtrk3Pt[i]<0.3&&abs(Dtrk1Eta[i])<1.2&&abs(Dtrk2Eta[i])<1.2&&abs(Dtrk3Eta[i])<1.2&&abs(Dy[i])<1&&Dtrk1highPurity[i]==1&&Dtrk2highPurity[i]==1&&Dtrk3highPurity[i]==1&&DlxyBS[i]/sqrt(DlxyBSErr[i])>dlscut&&(Dtrk1PixelHit[i]+Dtrk1StripHit[i])>=11&&(Dtrk2PixelHit[i]+Dtrk2StripHit[i])>=11&&(Dtrk3PixelHit[i]+Dtrk3StripHit[i])>=11&&Dgen[i]==23333&&Dchi2cl[i]>chi2cut&&proton_pt[i]/Dpt[i]>protoncut&&kaon_pt[i]/Dpt[i]>kaoncut)
				{
					weight=data_fit_fun->Eval(Dgenpt[i])/MC_fit_fun->Eval(Dgenpt[i]);
					h_alpha_10_20->Fill(alpha_BS_2D_correction[i],weight);
				}
	...
	divideBinWidth(h_alpha_4_5);
	divideBinWidth(h_alpha_5_6);
	divideBinWidth(h_alpha_6_8);
	divideBinWidth(h_alpha_8_10);
	divideBinWidth(h_alpha_10_125);
	divideBinWidth(h_alpha_125_15);
	divideBinWidth(h_alpha_15_20);
	divideBinWidth(h_alpha_10_20);
	h_alpha_4_5->Scale(1/h_alpha_4_5->Integral());
	h_alpha_5_6->Scale(1/h_alpha_5_6->Integral());
	h_alpha_6_8->Scale(1/h_alpha_6_8->Integral());
	h_alpha_8_10->Scale(1/h_alpha_8_10->Integral());
	h_alpha_10_125->Scale(1/h_alpha_10_125->Integral());
	h_alpha_125_15->Scale(1/h_alpha_125_15->Integral());
	h_alpha_15_20->Scale(1/h_alpha_15_20->Integral());
	h_alpha_10_20->Scale(1/h_alpha_10_20->Integral());
	/*
	...
	*/
	...
	h_alpha_6_8->Write();
	h_alpha_8_10->Write();
	h_alpha_10_125->Write();
	h_alpha_125_15->Write();
	h_alpha_15_20->Write();
	h_alpha_10_20->Write();
	outf->Write();
	outf->Close();
...
int main(int argc, char *argv[])
{
	if(argc==3)
	{
		draw_MC_5_6_alpha(argv[1], argv[2]);
	}
	else
	{
