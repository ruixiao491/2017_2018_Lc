...
int draw_MC_5_6_dls(TString infile="/scratch/halstead/x/xiao147/official_2017_18_MC/pp_Dntuple/with_dlsalpha_weight/pT10_MC_1020_combine/test_distribution/Delta1232_pt10_MC_BDTG_pT3_100_5_cutvariable_0_1_2_3_5_DATA.root",TString outfile="test_reweight_1020_extenf12.root",Bool_t iseos=false)
{
	...
	double edges[NBINS+1]={2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7,7.5,8,8.5,9,10,11,12};
	...
	for (int entry = 0; entry < t2->GetEntries(); entry++)
	{
		...
		if (abs(PVz)<15&&pBeamScrapingFilter&&pPAprimaryVertexFilter)
		{
			for (int i = 0; i < Dsize; i++ )
			{
				...
				{
					weight=data_fit_fun->Eval(Dgenpt[i])/MC_fit_fun->Eval(Dgenpt[i]);
					h_dls_10_20->Fill(DlxyBS[i]/sqrt(DlxyBSErr[i]),weight);
				}
