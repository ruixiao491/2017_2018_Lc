...
int draw_MC_5_6_kaon(TString infile="/scratch/halstead/x/xiao147/official_2017_18_MC/pp_Dntuple/with_dlsalpha_weight/pT10_MC_1020_combine/test_distribution/Delta1232_pt10_MC_BDTG_pT3_100_5_cutvariable_0_1_2_3_5_DATA.root",TString outfile="test_reweight_1020_extenf12.root",Bool_t iseos=false)
{
	...
	double edges[NBINS+1]={0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.78,0.8,0.82,0.84,0.86,0.9,0.92,0.95};
	...
	for (int entry = 0; entry < t2->GetEntries(); entry++)
	{
		...
		if (abs(PVz)<15&&pBeamScrapingFilter&&pPAprimaryVertexFilter)
		{
			for (int i = 0; i < Dsize; i++ )
			{
				...
				if(Dpt[i]>10&&Dpt[i]<20&&Dtrk1PtErr[i]/Dtrk1Pt[i]<0.3&&Dtrk2PtErr[i]/Dtrk2Pt[i]<0.3&&Dtrk3PtErr[i]/Dtrk3Pt[i]<0.3&&abs(Dtrk1Eta[i])<1.2&&abs(Dtrk2Eta[i])<1.2&&abs(Dtrk3Eta[i])<1.2&&abs(Dy[i])<1&&Dtrk1highPurity[i]==1&&Dtrk2highPurity[i]==1&&Dtrk3highPurity[i]==1&&DlxyBS[i]/sqrt(DlxyBSErr[i])>2&&(Dtrk1PixelHit[i]+Dtrk1StripHit[i])>=11&&(Dtrk2PixelHit[i]+Dtrk2StripHit[i])>=11&&(Dtrk3PixelHit[i]+Dtrk3StripHit[i])>=11&&Dgen[i]==23333&&Dchi2cl[i]>chi2cut&&alpha_BS_2D_correction[i]<alphacut&&kaon_pt[i]/Dpt[i]>kaoncut&&(DlxyBS[i]/sqrt(DlxyBSErr[i]))>dlscut)
				{
					weight=data_fit_fun->Eval(Dgenpt[i])/MC_fit_fun->Eval(Dgenpt[i]);
					h_dls_10_20->Fill(proton_pt[i]/Dpt[i],weight);
				}
...
int main(int argc, char *argv[])
{
	if(argc==3)
	{
		draw_MC_5_6_kaon(argv[1], argv[2]);
	}
	else
	{
