...
using namespace std;
  Bool_t istest = false;
  int data_triggerfireup_pp_eventselection_forrightdlBS_noterr_foreventfilter(TString infile="", TString outfile="", Bool_t iseos=false)
{
	if(istest)
	{
...
ntlambdaCtopkpi->SetBranchAddress("EvtNo",&EvtNo);
...
int main(int argc, char *argv[])
{
	if(argc==3)
	{
		data_triggerfireup_pp_eventselection_forrightdlBS_noterr_foreventfilter(argv[1], argv[2]);
	}
	else
	{
