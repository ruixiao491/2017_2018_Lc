...
Bool_t isPbPb;
//void readxml_lambda_C(TString weightfile, TString collisionsyst, Float_t ptMin, Float_t ptMax)
void readxml_lambda_C(TString weightfile="/home/xiao147/private/TMVA_lambdaC/CMSSW_7_5_8_patch3/src/tmvaDntupleRunII_try_BDT/myTMVA/weights/TMVAClassification_BDT.weights.xml", TString collisionsyst="pp", Float_t ptMin=6.0, Float_t ptMax=20.0)
{
  ...
  for(unsigned int k=0;k<nVar;k++)
    {
      ...
      varval[k] = varname;
 //    cout<<"number of training variables"<<nVar<<endl;
  ...
  while(eff)
      ...
      for(ULong_t l=0;l<varnames.size();l++)
	{
	  ...
	  //cout<<"margins[l]: "<<margins[l]<<"max: "<<max<<"min: "<<min<<endl;
	  if(margins[l]=="FMin")
	    {
...
//	wBackground = 3592760;
  ...
  Double_t max = wSignal*effS[1]/sqrt(wBackground*effB[1]);
  int maxindex = 1;
  effS[0]=0;
  for(int i=1;i<100;i++)
    {
     	//effSig[i] = wSignal*effS[i]/sqrt(wSignal*effS[i]+wBackground*effB[i]);
      //change to the new defination
  ...
  cout<<"  --------------------------------------------------"<<endl;
//cout<<"test"<<cutval[2].at(maxindex)<<endl;
 //cout<<"test length: "<<sizeof(cutval)/sizeof(cutval[0])<<endl;
// cout<<"maxindex"<<maxindex<<endl;
// cout<<"test again"<<sizeof(cutval[2])<<endl;
 //check
 /*
 for (int i=0; i<100; i++)
 {
	 cout<<"test val1"<<cutval[1].at(i)<<endl;
	 cout<<"test val2"<<cutval[2].at(i)<<endl;
 }
 */
 for(unsigned int m=0;m<nVar;m++)
 // for(unsigned int m=0;m<2;m++)
    {
	 // cout<<"m test: "<<m<<"value: test "<<cutval[m]<<endl;
	 // cout<<"m: "<<m<<"value: "<<cutval[m].at(maxindex)<<endl;
      if(m) cout<<"  --------------------------------------------------"<<endl;
  ...
  hempty->GetYaxis()->SetTitle("S/#sqrt{B}");
  ...
  texPtY->Draw();
  gsig->Draw("same*");
  csig->SaveAs(Form("plots/Significance_%s_%s_%.1f_%.1f_noalpha_inCfiles_setRange3.pdf",colsyst.Data(),mvafun.Data(),ptmin,ptmax));
}
/*
void calRatio(Float_t* results, Bool_t verbose=false)
{
