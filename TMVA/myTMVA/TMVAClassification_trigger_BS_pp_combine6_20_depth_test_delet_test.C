...
#include "../prefilter_modify_trigger_sidebandchange_withBBS.h" //PbPb sideband data as training samples
//#include "../prefilter_modify_trigger_sidebandchange_withBBS_changeB.h" //PbPb MC signal region background samples
#include <vector>
#include <sstream>
#include "string.h"
#include <iostream>

//void TMVAClassification_trigger_BS_pp_combine6_20(TString collisionsyst="pp", float ptMin=10.0, float ptMax=20.0, TString myMethodList = "CutsSA", int cut_number=3, std::string cut_list_number={"0,3,4"}, TString name_variable = "0_3_4")
void TMVAClassification_trigger_BS_pp_combine6_20_depth_test_delet_test(TString collisionsyst, float ptMin, float ptMax, TString myMethodList, int cut_number, std::string cut_list_number, TString name_variable)
{
  Bool_t isPbPb;
  ...
  TString outfileName(Form("ROOT/TMVA_%s_%.0f_%.0f_%s_cut_%s.root",colsyst.Data(),ptmin,ptmax,myMethodList.Data(),name_variable.Data()));
  TString outputstr(Form("TMVAClassification_%s_%.0f_%.0f_%s_%i_cutvariables_%s_weights",colsyst.Data(),ptmin,ptmax,myMethodList.Data(),cut_number,name_variable.Data()));
  ...
  // [all types of expressions that can also be parsed by TTree::Draw( "expression" )]
    ...
    gSystem->Exec(Form("mkdir -p dataset/weights/%s", outputstr.Data()));
	(TMVA::gConfig().GetIONames()).fWeightFileDir = Form("weights/%s", outputstr.Data());
...
*/
	...
	std::vector<string> cut_list;
	//cut_list.push_back("(DlxyBS/sqrt(DlxyBSErr))");//0
	cut_list.push_back("(log10(DsvpvDistance/DsvpvDisErr))");//0
	cut_list.push_back("Dchi2cl");//1
	//cut_list.push_back("alpha_BS_2D_correction");//2
	cut_list.push_back("Dalpha");//2
	cut_list.push_back("kaon_pt/Dpt");//3
	cut_list.push_back("pion_pt/Dpt");//4
	cut_list.push_back("proton_pt/Dpt");//5
	cut_list.push_back("Dy");//6
	cut_list.push_back("Dtrk1Eta");//7
	cut_list.push_back("Dtrk2Eta");//8
	cut_list.push_back("Dtrk3Eta");//9
	//cut_list.push_back("(DsvpvDistance/DsvpvDisErr)");//10
	//cut_list.push_back("Dalpha");//11

int numbers_chosen[cut_number];
int j=0;
	/////here to find the chosen variables
	for (int i=0;i<cut_list_number.size();i++)
	{   
		char an[]={','};
		if (cut_list_number.at(i)!= int(*an))
		{   
			cout<<"print variable index first turn"<<cut_list_number.at(i)<<endl;
			stringstream geek;
			geek << cut_list_number.at(i);
		...
		{   
			cout<<"No registered variables"<<i<<cut_list_number.at(i)<<endl;
		}   
	...
	for (int i=0; i<cut_number; i++)
	{
		...
		cout<<"chosen variable index second turn"<<a<<endl;
		factory->AddVariable(cut_list[a]);

	}

//int a = numbers_chosen[0];
//cout<<a<<endl;
//factory->AddVariable(cut_list[a]);//>
//factory->AddVariable("(DlxyBS/sqrt(DlxyBSErr))");//>
  ...
  TTree* background = (TTree*)inputB->Get("t1");
 ...
 factory->PrepareTrainingAndTestTree( mycutS, mycutB, "nTrain_Signal=0:nTrain_Background=0:SplitMode=Random:NormMode=NumEvents:!V" );
   ...
   if (Use["CutsSA"])
      factory->BookMethod( TMVA::Types::kCuts, "CutsSA",
                           //"!H:!V:FitMethod=SA:EffSel:MaxCalls=150000:KernelTemp=IncAdaptive:InitialTemp=1e+6:MinTemp=1e-6:Eps=1e-10:UseDefaultScale:VarProp[0]=FMax:VarProp[1]=FMin:VarProp[2]=FMax:VarProp[3]=FMax:VarProp[4]=FMax" );
                           //"!H:!V:FitMethod=SA:EffSel:MaxCalls=150000:KernelTemp=IncAdaptive:InitialTemp=1e+6:MinTemp=1e-6:Eps=1e-10:UseDefaultScale:VarProp[0]=FMax:VarProp[1]=FMin:VarProp[2]=FMax:VarProp[3]=FMax:VarProp[4]=FMin" );
   ...
   if (Use["BDT"])  // Adaptive Boost
      factory->BookMethod( TMVA::Types::kBDT, "BDT",
                           "!H:!V:NTrees=800:MinNodeSize=2.5%:MaxDepth=3:BoostType=AdaBoost:AdaBoostBeta=0.5:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=GiniIndex:nCuts=30" );
