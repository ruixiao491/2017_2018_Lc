...
int make_tuple_for_Roofit_alldata_pp_agreedMC_noalpha_uniform_cutsgetfromKevin_3_4(TString infile, TString outfile,  Bool_t iseos=false)
//int make_tuple_for_Roofit_alldata_pp_agreedMC_noalpha_uniform_cutsgetfromKevin_3_4(TString infile="/scratch/halstead/x/xiao147/2017_pp_firstturn_dataproduction_0416_2019/fill_BDT/pp_3_4_log_dls/HIZero1/Dntuple_Dntuple_Dntuple_HiForestAOD_1000.root_993.root_3.root_0.root", TString outfile="test.root",  Bool_t iseos=false)
{
	...
	TNtuple *nt1 = new TNtuple("nt1", "pp_Kevin_3_4_nopion_noprecuts_BDT","mass");
	TNtuple *nt2 = new TNtuple("nt2", "pp_Kevin_3_4_nopion_noprecuts_rect","mass");
    ...
    double BDT_pt_3_20_5_cutvar_0_1_2_3_5_weight[MAX_XB];
	float DlxyBS[MAX_XB];
	...
	mvaTree->SetBranchAddress("BDT_pt_3_20_5_cutvar_0_1_2_3_5_weight",BDT_pt_3_20_5_cutvar_0_1_2_3_5_weight);
	...
	for (int entry = 0; entry < t2->GetEntries(); entry++)
	{
		...
		if (abs(PVz)<15&&pBeamScrapingFilter&&pPAprimaryVertexFilter&&pass12trigger)
		{
			for (int i = 0; i < Dsize; i++ )
			{
				...
				if(BDT_pt_3_20_5_cutvar_0_1_2_3_5_weight[i]>0&&Dpt[i]>3&&Dpt[i]<4&&Dtrk1PtErr[i]/Dtrk1Pt[i]<0.3&&Dtrk2PtErr[i]/Dtrk2Pt[i]<0.3&&Dtrk3PtErr[i]/Dtrk3Pt[i]<0.3&&abs(Dtrk1Eta[i])<1.5&&abs(Dtrk2Eta[i])<1.5&&abs(Dtrk3Eta[i])<1.5&&abs(Dy[i])<1&&Dtrk1highPurity[i]==1&&Dtrk2highPurity[i]==1&&Dtrk3highPurity[i]==1&&DlxyBS[i]/sqrt(DlxyBSErr[i])>2)
				{
					nt1->Fill(Dmass[i]);
				}
				if(Dpt[i]>3&&Dpt[i]<4&&Dtrk1PtErr[i]/Dtrk1Pt[i]<0.3&&Dtrk2PtErr[i]/Dtrk2Pt[i]<0.3&&Dtrk3PtErr[i]/Dtrk3Pt[i]<0.3&&abs(Dtrk1Eta[i])<1.5&&abs(Dtrk2Eta[i])<1.5&&abs(Dtrk3Eta[i])<1.5&&abs(Dy[i])<1&&Dtrk1highPurity[i]==1&&Dtrk2highPurity[i]==1&&Dtrk3highPurity[i]==1&&log10(DlxyBS[i]/sqrt(DlxyBSErr[i]))>2.8342313072366954e-01&&Dchi2cl[i]>5.7872181175158605e-02&&alpha_BS_2D_correction[i]<3.2552652103126356e-01&&kaon_pt[i]/Dpt[i]>0.133&&proton_pt[i]/Dpt[i]>0.245)
				{   
					nt2->Fill(Dmass[i]);
				}
...
int main(int argc, char *argv[])
{
	if(argc==3)
	{
		make_tuple_for_Roofit_alldata_pp_agreedMC_noalpha_uniform_cutsgetfromKevin_3_4(argv[1], argv[2]);
	}
	else
	{
